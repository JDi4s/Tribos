(function () {
    ...
    class NobresCalculator {
        static translations() {
            return {
                ...
                en_US: {
                    ...
                    detectionNote: 'Academy values are auto-detected. You can still adjust them manually if needed.',
        ...
        async #getWorldConfig() {
            ...
            await this.#waitMilliseconds(Date.now(), 80);
            return xmlString;
        }
        ...
        #sumVillageObjects(obj) {
            ...
            Object.keys(obj || {}).forEach(id => {
                const v = obj[id] || {};
                totals.wood += Number(v.wood || 0);
                totals.stone += Number(v.stone || 0);
                totals.iron += Number(v.iron || 0);
            });

            return totals;
        }

        #getTotalsFromBalancer() {
            const hasVillageArrays =
                Array.isArray(window.allWoodTotals) &&
                Array.isArray(window.allClayTotals) &&
                Array.isArray(window.allIronTotals);

            if (!hasVillageArrays) return null;

            let wood = 0;
            let stone = 0;
            let iron = 0;

            for (let i = 0; i < window.allWoodTotals.length; i++) {
                wood += this.#parseNumber(window.allWoodTotals[i]);
            }
            for (let i = 0; i < window.allClayTotals.length; i++) {
                stone += this.#parseNumber(window.allClayTotals[i]);
            }
            for (let i = 0; i < window.allIronTotals.length; i++) {
                iron += this.#parseNumber(window.allIronTotals[i]);
            }

            const incoming = { wood: 0, stone: 0, iron: 0 };

            if (window.incomingRes && typeof window.incomingRes === 'object') {
                Object.keys(window.incomingRes).forEach(villageId => {
                    const v = window.incomingRes[villageId] || {};
                    incoming.wood += Number(v.wood || 0);
                    incoming.stone += Number(v.stone || 0);
                    incoming.iron += Number(v.iron || 0);
                });
            }

            return {
                totalResources: { wood, stone, iron },
                incomingResources: incoming,
                villagesRead: window.allWoodTotals.length
            };
        }

        async #loadData() {
            const balancerTotals = this.#getTotalsFromBalancer();

            if (balancerTotals) {
                this.totalResources = balancerTotals.totalResources;
                this.incomingResources = balancerTotals.incomingResources;
                this.villagesRead = balancerTotals.villagesRead;
            } else {
        ...
        async #getVillageResources() {
            ...
            rows.each((_, row) => {
                ...
                resourcesByVillage[String(villageId)] = {
                    wood: this.#parseNumber(resourcesCell.find('.res.wood, .wood').text()),
                    stone: this.#parseNumber(resourcesCell.find('.res.stone, .stone').text()),
                    iron: this.#parseNumber(resourcesCell.find('.res.iron, .iron').text())
                };
            });
        ...
        async #getIncomingResourcesByVillage() {
            ...
            for (const type of types) {
                await this.#setMaxLinesPerPage('overview_villages', 'trader', 1000, { type });
                await this.#waitMilliseconds(Date.now(), 100);

                const rawPage = this.#fetchHtmlPage(
                    this.#generateUrl('overview_villages', 'trader', { type, page: 0 })
                );
                if (!rawPage) continue;
                ...
                rows.each((_, row) => {
                    ...
                    if (!targetVillageId) {
                        targetVillageId = 'global';
                    }

                    const resourcesCell = cells.eq(cells.length - 1);
                    const txt = resourcesCell.text().replace(/\./g, '').replace(',', ' ');
                    const numbers = txt.match(/\d+/g) || [];

                    const wood = parseInt(numbers[0], 10) || 0;
                    const stone = parseInt(numbers[1], 10) || 0;
                    const iron = parseInt(numbers[2], 10) || 0;
        ...
        async #getSnobData() {
            ...
            return {
                ...
                nextNobleCoins: savedForNextNoble,
                existingCoins: totalMintedCoins,
        ...
        #buildSummaryText(plan, detected) {
            return [
                ...
                '',
                `${this.t.totalResources}:`,
                `${this.t.wood}: ${this.#formatNumber(this.totalResources.wood)}`,
                `${this.t.stone}: ${this.#formatNumber(this.totalResources.stone)}`,
                `${this.t.iron}: ${this.#formatNumber(this.totalResources.iron)}`,
                '',
                `${this.t.incomingResources}:`,
                `${this.t.wood}: ${this.#formatNumber(this.incomingResources.wood)}`,
                `${this.t.stone}: ${this.#formatNumber(this.incomingResources.stone)}`,
                `${this.t.iron}: ${this.#formatNumber(this.incomingResources.iron)}`,
                '',
...
`;
            ...
            $(document).on('click.' + SCRIPT_NS, '#nc-copy-summary', async () => {
                const summary = this.#buildSummaryText(plan, detected);
                try {
