(function () {
    ...
    var SCRIPT_NS = 'defesa_disponivel_merged';
    ...
    class VillagesTroopsCounter {
        static translations() {
            return {
                en_US: {
                    ...
                    credits: 'Merged Troops Counter by JDi4s'
                },
                pt_PT: {
                    ...
                    credits: 'Contador de tropas fundido por JDi4s'
                }
            };
        }
        ...
        constructor() {
            ...
            this.availableSupportUnits = Array.isArray(game_data.units) ? [...game_data.units] : [];
        ...
        #buildDefensiveTroopsObj(troopsObj) {
            const total = this.#buildTotalTroopsObj(troopsObj);
            return {
                spear: total.spear || 0,
                sword: total.sword || 0,
                archer: total.archer || 0,
                heavy: total.heavy || 0,
                spy: total.spy || 0,
                catapult: total.catapult || 0,
                knight: total.knight || 0
            };
        }
        ...
        #sendToDiscord(totalTroops, defensiveTroops, nukeStatus) {
            const validWebhook =
                ...
                embeds: [
                    {
                        ...
                        color: 3066993,
                        fields: [
                            { name: "Lanceiros", value: `${this.#formatNumber(defensiveTroops.spear)}`, inline: true },
                            { name: "Espadachins", value: `${this.#formatNumber(defensiveTroops.sword)}`, inline: true },
                            { name: "Arqueiros", value: `${this.#formatNumber(defensiveTroops.archer)}`, inline: true },
                            { name: "Batedores", value: `${this.#formatNumber(defensiveTroops.spy)}`, inline: true },
                            { name: "C. Pesada", value: `${this.#formatNumber(defensiveTroops.heavy)}`, inline: true },
                            { name: "Catapultas", value: `${this.#formatNumber(defensiveTroops.catapult)}`, inline: true },
                            { name: "Paladino", value: `${this.#formatNumber(defensiveTroops.knight)}`, inline: true }
                        ]
                    },
                    {
                        ...
                        color: 15158332,
                        fields: [
                            { name: "🚀 Fulls (19k+)", value: `${nukeStatus.full}`, inline: true },
                            { name: "📈 Semis (10k+)", value: `${nukeStatus.semi}`, inline: true },
                            { name: "🛠️ Recrutar", value: `${nukeStatus.rebuilding}`, inline: true }
                        ]
                    },
                    {
                        title: "📦 TROPA TOTAL",
                        color: 3447003,
                        fields: [
                            { name: "Machados", value: `${this.#formatNumber(totalTroops.axe || 0)}`, inline: true },
                            { name: "Leves", value: `${this.#formatNumber(totalTroops.light || 0)}`, inline: true },
        ...
        <table class="vis overview_table" width="100%">
            ...
            <tbody>
                <tr style="text-align:center">
                    <td>${this.#formatNumber(defensiveTroops.spear)}</td>
                    <td>${this.#formatNumber(defensiveTroops.sword)}</td>
                    ${game_data.units.includes('archer') ? `<td>${this.#formatNumber(defensiveTroops.archer)}</td>` : ''}
                    <td>${this.#formatNumber(defensiveTroops.spy)}</td>
                    <td>${this.#formatNumber(defensiveTroops.heavy)}</td>
                    <td>${this.#formatNumber(defensiveTroops.catapult)}</td>
                    ${game_data.units.includes('knight') ? `<td>${this.#formatNumber(defensiveTroops.knight)}</td>` : ''}
                </tr>
            </tbody>
        </table>
    </div>
...
<style>
.popup_box_content { min-width: 760px; }
...
`;
            ...
            $(document).on('click.' + SCRIPT_NS, '#dd-send-discord', () => {
                this.#sendToDiscord(totalTroops, defensiveTroops, nukeStatus);
            });
